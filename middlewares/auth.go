...
func Auth() gin.HandlerFunc{
	return func(context *gin.Context) {
		...
		// Ideally, we expect the token to be sent as a header by the client. 
		// If there are no tokens found at the header, the application throws a 401 error with the appropriate error message.
		...
		// Here, we validate the token using the earlier created helper function. 
