...
// Here we set a default expiration time as 1 Hour, which can be (and should be) made configurable. 
// From there on we create a new claim variable with the available data and expiration time. 
...
// Here, in the ValidateToken() function, we would take in the token string coming from the client’s HTTP request header and validate it. 
// So, here we will try to parse the JWT into claims using the JWT package’s helper method “ParseWithClaims”. 
// From the parsed token, we extract the claims at Line 48. 
// Using these claims, we check at Line 53 if the token is actually expired or not. 
